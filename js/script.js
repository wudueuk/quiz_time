...
buttonClear.className = 'main__btn btn__clear';
buttonClear.textContent = 'Очистить результаты';
...
const checkStorage = () => {
  if (localStorage.length > 0 && buttonClear.style.display === 'none') {
...
const renderQuiz = quiz => {
  ...
  const showQuestion = () => {
    ...
    form.addEventListener('submit', event => {
      ...
      if (ok) {
        if (answer.every((result, i) => !!result === answersData.keys[i])) result++;
